...
public class ReplyInsertService implements ReplyService {
	...
	public ModelAndView execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(result >0) {
			//댓글이 삽입되면 댓글리스트를 DB에서 새로 가져와야한다.
			//따라서 댓글리스트를 DB에서 가져오는 서비스인 NoticeViewService를 실행할수 있는"view.notice"매핑으로 이동한다.
			//"view.notice"로 redirect이동함 (삽입, 수정, 삭제 이후에는 redirect)
			
			//"veiw.notice"이동은 항상 nNo를 파라미터로 전달해야함.
